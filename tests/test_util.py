...
import pytest
from steam2sqlite.utils import delay_by
...
def test_delay_by():
    ...
    delay_time = 0.25
    begin = time.monotonic()
    delay_by(delay_time)(slow_hello)()  # takes 0.5 sec
    dur = time.monotonic() - begin
    assert dur > delay_time
    assert dur == pytest.approx(0.5, rel=0.1)
